...
const App = () => {
  const [position, setPosition] = useState(null); // live location
  const [error, setError] = useState(null);
  const [capturedUrl, setCapturedUrl] = useState(null);
  const [captures, setCaptures] = useState([]); // map markers
  const [isOnline, setIsOnline] = useState(navigator.onLine);
  const [ShowInfo, setshowInfo] = useState(false);
  const [showLocationModal, setShowLocationModal] = useState(false);
  const watchIdRef = useRef(null);
  ...
  const handleBackdropClick = (e) => {
    if (e.target === modalRef.current) setShowLocationModal(false);
  };
  ...
  return (
    <>
      ...
      <button className="route-btn" onClick={() => setShowLocationModal((prev) => !prev)}>Route</button>
      {ShowInfo && <Mobileerror />}
